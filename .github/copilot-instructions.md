...
10. Copilot コードレビュー対応（初回レビューのみ、最大3回の指摘対応）
    ...
    - **プッシュ後 GitHub CI 再確認**（必須）: 再プッシュ後に `gh pr checks <PR_NUMBER>` でチェック結果を確認する。CI が通過しない・無応答の場合は PR 状態診断フロー（`agents/orchestrator.agent.md` 参照）でコンフリクト等の原因を調査・解消し、解消後に通常フローに戻る
