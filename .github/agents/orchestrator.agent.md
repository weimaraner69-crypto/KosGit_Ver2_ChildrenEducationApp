...
18. PR の CI 結果を確認する（60秒間隔 × 最大20回 = 最大20分）：
    ```bash
    for i in $(seq 1 20); do
      echo "CI 確認 ($i/20)..."; sleep 60
      STATUS=$(gh pr checks <PR番号> --json name,status,conclusion \
        --jq '[.[] | {name,status,conclusion}]')
      echo "$STATUS"
      FAILED=$(echo "$STATUS" | jq '[.[] | select(.conclusion=="failure" or .conclusion=="cancelled")] | length')
      PENDING=$(echo "$STATUS" | jq '[.[] | select(.status=="queued" or .status=="in_progress")] | length')
      if [ "$FAILED" -gt 0 ]; then echo "❌ CI 失敗"; break; fi
      if [ "$PENDING" -eq 0 ]; then echo "✅ 全チェック通過"; break; fi
    done
    ```
19. **CI が失敗した場合**：
    - エラー内容を取得する
...
2. 初回 Copilot レビューの到着を待機する
   ...
   - タイムアウト（20分）した場合はスキップして Step 10 へ進む
